...
def create_partition_map(region: np.ndarray, elevation: np.ndarray, landcover: np.ndarray, hazard: np.ndarray, seed: int = 42, train_ratio = 0.8) -> np.ndarray:
    ...
    # only deal with regions in valid terrain and remove water from landcover

    region[np.isnan(elevation)] = 0
    region[landcover == 210] = 0  # Example water types in landcover
    ...
    print(f"Found {len(unique_regions)} regions with hazard occurrences")
    ...
    if len(unique_regions) > 0:
        ...
        train_ids = unique_regions[:int(train_ratio * n)]
        val_ids = unique_regions[int(train_ratio * n):int((train_ratio+val_ratio) * n)]
        test_ids = unique_regions[int((train_ratio+val_ratio) * n):]
...
def erode_partition_borders(partition_map: np.ndarray, kernel_size: int = 5) -> np.ndarray:
    ...
    eroded_map = np.zeros_like(partition_map, dtype=np.uint8)
...
def balance_partition_map(partition_map: np.ndarray, labels: np.ndarray, seed: int = 42) -> np.ndarray:
    """
    Balances each split (train/val/test) by downsampling negative (label=0) cells 
    to match the number of positive (label=1) cells within each partition (train/valid/test).
    ...
    for split in [1, 2, 3]:
        ...
        region_indices = np.flatnonzero(split_mask)
...
def sample_partition_map(partition_map: np.ndarray, size = 1, seed: int = 42) -> np.ndarray:
    ...
    if size > total_size:
        ...
        # Add the borders back in
        border_mask = (partition_map == 4)
        sampled_partition_map[border_mask] = 4

        return sampled_partition_map
    
def validate_inputs(args):
    valid_hazards = ['flood', 'wildfire', 'landslide']
    if args.hazard not in valid_hazards:
        print(f"Error: Invalid hazard '{args.hazard}'. Valid options are: {valid_hazards}")
        sys.exit(1)
if __name__ == "__main__":
    
    # Command-line arguments
    argparse = argparse.ArgumentParser(description="Create partition map for a specific hazard in Europe.")
    argparse.add_argument("-z", "--hazard", type=str, help="Name of the hazard to create partition map for (e.g., flood, wildfire, landslide).")
    ...
    # Load data     
    sub_countries = np.load(partition_paths["sub_countries"])
    hazard = np.load(label_paths[hazard_name])
    ...
    partition_map = create_partition_map(region=sub_countries, elevation=elevation, landcover=landcover, hazard=hazard, seed=seed)
    ...
    plot_npy_arrays(partition_map, name="partition",title=f"{hazard_name.capitalize()} Partition Map", type="partition", downsample_factor=10, save_path=f"{partition_paths_downscaled[hazard_name][:-4]}.png")
